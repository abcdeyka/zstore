...
class FinReportSmall extends \App\Pages\Base
{
    ...
    private function getData() {
        ...
        $header = array(
            ...
            'edrpou' => (string) sprintf("%10d", $firm['tin'] ?? 0),
            'koatuu' => (string) sprintf("%10d", $firm['koatuu'] ?? 0),
            'kopfg' => (string) sprintf("%10d", $firm['kopfg'] ?? 0),
            'kodu' => (string) sprintf("%10d", $firm['kodu'] ?? 0),
            'kved' => (string) sprintf("%10s", $firm['kved'] ?? ''),
            //без дужок ?? має нижчий пріоритет за конкатенацію, і місто з телефоном губились
